...
- **Confirmation rule:** draft only; the CEO reviews and sends manually.
...
3. **OAuth consent screen**
   - Add `you@togohealth.com` as a test user (or publish internally under the Workspace org)
...
5. **First run** â€” authorizes once (opens a browser), saves `token.json`:
   ```
   python3 welcome_draft.py --facility "Test Building" --to "you@togohealth.com"
   ```
...
â†’ Draft appears in you@togohealth.com Drafts. Review, confirm recipients, send.
